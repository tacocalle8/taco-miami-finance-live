...
export async function ensureCloverSchema() {
  if (!cloverSchemaPromise) {
    ...
    cloverSchemaPromise = (async () => {
      ...
      `;
      await sql`ALTER TABLE clover_payments ADD COLUMN IF NOT EXISTS tender_type TEXT`;
      await sql`ALTER TABLE clover_payments ADD COLUMN IF NOT EXISTS tender_label TEXT`;
  ...
  for (let index = 0; index < payments.length; index += 40) {
    ...
    await Promise.all(batch.map(payment => sql`
      INSERT INTO clover_payments (
        merchant_id, payment_id, store_key, order_id, tender_id, tender_type, tender_label, amount_cents, tip_cents, tax_cents,
        ...
        ${payment.tenderType}, ${payment.tenderLabel},
...
export async function aggregateCloverActivity(startMs, endMs) {
  ...
  const payments = await sql`
    SELECT store_key, 'SUCCESS' AS result,
           ...
           COALESCE(SUM(amount_cents) FILTER (WHERE LOWER(CONCAT_WS(' ', tender_type, tender_label)) LIKE '%cash%'), 0) AS cash_amount_cents,
           COALESCE(SUM(tip_cents) FILTER (WHERE LOWER(CONCAT_WS(' ', tender_type, tender_label)) LIKE '%cash%'), 0) AS cash_tip_cents,
           COALESCE(SUM(tax_cents) FILTER (WHERE LOWER(CONCAT_WS(' ', tender_type, tender_label)) LIKE '%cash%'), 0) AS cash_tax_cents,
           COALESCE(SUM((surcharge_cents + convenience_fee_cents + other_charge_cents)) FILTER (WHERE LOWER(CONCAT_WS(' ', tender_type, tender_label)) LIKE '%cash%'), 0) AS cash_fees_cents,
           COUNT(*) FILTER (WHERE LOWER(CONCAT_WS(' ', tender_type, tender_label)) LIKE '%cash%') AS cash_transaction_count,
  ...
  `;
  const refunds = await sql`
    SELECT r.store_key, 'SUCCESS' AS result,
           COALESCE(SUM(r.amount_cents), 0) AS amount_cents,
           COALESCE(SUM(r.tip_cents), 0) AS tip_cents,
           COALESCE(SUM(r.tax_cents), 0) AS tax_cents,
           COALESCE(SUM(r.surcharge_cents), 0) AS surcharge_cents,
           COALESCE(SUM(r.convenience_fee_cents), 0) AS convenience_fee_cents,
           COALESCE(SUM(r.other_charge_cents), 0) AS other_charge_cents,
           COALESCE(SUM(r.amount_cents) FILTER (WHERE LOWER(CONCAT_WS(' ', p.tender_type, p.tender_label)) LIKE '%cash%'), 0) AS cash_refund_amount_cents,
           COALESCE(SUM(r.tax_cents) FILTER (WHERE LOWER(CONCAT_WS(' ', p.tender_type, p.tender_label)) LIKE '%cash%'), 0) AS cash_refunded_tax_cents,
           COALESCE(SUM(r.surcharge_cents + r.convenience_fee_cents + r.other_charge_cents) FILTER (WHERE LOWER(CONCAT_WS(' ', p.tender_type, p.tender_label)) LIKE '%cash%'), 0) AS cash_refunds_fees_cents,
           COALESCE(SUM(r.amount_cents + r.tip_cents + r.surcharge_cents + r.convenience_fee_cents + r.other_charge_cents) FILTER (WHERE LOWER(CONCAT_WS(' ', p.tender_type, p.tender_label)) LIKE '%cash%'), 0) AS cash_refunds_cents,
           COUNT(*) FILTER (WHERE LOWER(CONCAT_WS(' ', p.tender_type, p.tender_label)) LIKE '%cash%') AS cash_refund_count,
           COUNT(*) AS refund_count
    FROM clover_refunds r
    LEFT JOIN clover_payments p ON p.merchant_id = r.merchant_id AND p.payment_id = r.payment_id
    WHERE r.created_time >= ${startMs} AND r.created_time < ${endMs}
      AND (r.result IS NULL OR r.result = 'SUCCESS')

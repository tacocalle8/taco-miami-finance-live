...
export function dateFilterPath(merchantId, resource, startMs, endMs, offset) {
  ...
  // Clover's payment and refund APIs expect a date range as one filter
  // expression joined with AND. Repeating the filter query can return an
  // empty collection even though the request itself succeeds.
  query.set('filter', `createdTime>=${startMs} AND createdTime<=${endMs}`);
...
export function buildCloverSummary(merchants, payments, refunds) {
  ...
  for (const refund of refunds) {
    ...
    const returned = Number(refund.amount_cents) + Number(refund.tip_cents) + fees;

...
function encryptionKey() {
  ...
  if (!source) {
    const error = new Error('Falta TOKEN_ENCRYPTION_KEY o PLAID_SECRET para cifrar el token bancario.');
    error.statusCode = 503;
    throw error;
  }
...
export function decryptToken(value) {
  ...
  if (!ivText || !tagText || !encryptedText) throw new Error('El token bancario guardado no es válido.');

...
export async function upsertItemAndAccounts({ itemId, accessToken, institution, accounts }) {
  ...
  // A new Plaid authorization for the same Wells Fargo login creates a new
  // Item and new account_ids. Keep the freshly authorized Item active and
  // deactivate the older Taco Miami Wells Fargo connection when it contains
  // the same three known accounts.
  await deactivateDuplicateWellsFargoItems(itemId);
}

export async function deactivateDuplicateWellsFargoItems(preferredItemId = null) {
  await ensureSchema();
  const sql = getSql();
  const preferred = preferredItemId || '';
  const deactivatedItems = await sql`
    WITH target_items AS (
      SELECT
        i.item_id,
        i.created_at,
        i.updated_at,
        ARRAY_AGG(COALESCE(a.mask, '') ORDER BY COALESCE(a.mask, '')) AS account_masks
      FROM plaid_items i
      JOIN plaid_accounts a ON a.item_id = i.item_id
      WHERE i.status = 'active'
        AND LOWER(i.institution_name) LIKE '%wells fargo%'
      GROUP BY i.item_id, i.created_at, i.updated_at
      HAVING COUNT(*) = 3
        AND ARRAY_AGG(COALESCE(a.mask, '') ORDER BY COALESCE(a.mask, ''))
          = ARRAY['0048', '4258', '8370']::TEXT[]
    ), ranked_items AS (
      SELECT
        item_id,
        ROW_NUMBER() OVER (
          ORDER BY
            CASE WHEN item_id = ${preferred} THEN 0 ELSE 1 END,
            updated_at DESC,
            created_at DESC,
            item_id DESC
        ) AS duplicate_rank
    ...
    WHERE item_id IN (
      SELECT item_id FROM ranked_items WHERE duplicate_rank > 1
    )
    RETURNING item_id
  `;

  return deactivatedItems.length;
}

export async function listAccounts() {
  // This also performs the one-time cleanup for connections that were saved
  // before duplicate prevention was added.
  await deactivateDuplicateWellsFargoItems();
  const sql = getSql();
  return sql`
